...
Guidance for AI coding agents working in `public-dkr`. It applies to the whole repository.

## Orientation

GitHub Actions matrix that pulls images from Docker Hub / gcr.io and pushes them to the org's public ECR (public.ecr.aws/b2i4h4k9). This is the mirror step between saas-gateway's Docker Hub build and the sha cvhome pins, and it supplies node, postgres, otel-contrib, zipkin, paketo and distroless base images. Add or bump an image = edit the matrix and push main.

Part of the `cvhome-saas` organisation. Cross-repo routing, review and releases live in
`cvhome-saas/orchestrator`; this file is the repo's own rulebook. Entry points: .github/workflows/push-images.yml, README.md.
